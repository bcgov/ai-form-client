...
export function createLauncher({ root, content = LAUNCHER_CONTENT, notice = null }) {
    ...
    function hideTooltip() {
        ...
        // Hold the hover rule off until the pointer has actually left. The dismiss
        // button sits inside the launcher, so without this the bubble is hidden and
        // handed straight back by :hover in the same frame, and the X does nothing
        // visible but remove itself.
    ...
    if (dismissButton) {
        dismissButton.addEventListener('click', () => {
            // Blurred before it is hidden. The click focuses it, and a focused child
            // keeps :focus-within true on the wrapper - which is the same rule handing
            // the bubble back, by its other half. The browser drops the focus once the
            // button stops rendering, but only after a frame in which the bubble has
            // already flashed back.
            dismissButton.blur();
            hideTooltip();
        });
    }
